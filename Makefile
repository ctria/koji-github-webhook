...
clean:
	rm -f *.o *.so *.pyc *~ koji-github-webhook*.bz2 koji-github-webhook*.src.rpm
	rm -rf koji-github-webhook-$(VERSION)
...
test-tarball:
	@rm -rf .koji-github-webhook-$(VERSION)
	@mkdir .koji-github-webhook-$(VERSION)
	@cp -al [A-Za-z]* .koji-$(VERSION)
	@mv .koji-github-webhook-$(VERSION) koji-github-webhook-$(VERSION)
	tar --bzip2 --exclude '*.tar.bz2' --exclude '*.rpm' --exclude '.#*' \
	    -cpf koji-github-webhook-$(VERSION).tar.bz2 koji-github-webhook-$(VERSION)
	@rm -rf koji-github-webhook-$(VERSION)
